...
namespace Math
{
	namespace MathUtil
	{
		...
		inline unsigned short ClampI(unsigned short val, unsigned short min, unsigned short max)
		{
		...
		inline double Lerp(double val, double min, double max)
		{
			return (1 - val) * min + val * max;
		}

		inline uint8_t LerpB(double val, uint8_t min, uint8_t max)
		{
